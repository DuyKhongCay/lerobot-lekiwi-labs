...
>>>>>>> 9e95a9325284e42689995885da3b36011644005e
...
<<<<<<< HEAD
    parser.add_argument(
        "--robot-port", required=True, help="Serial port of the SO-101 follower arm."
    )
    parser.add_argument(
        "--robot-id",
        default="so101_gamepad_follower",
        help="Calibration id for the robot.",
    )
=======
...
>>>>>>> 9e95a9325284e42689995885da3b36011644005e
    ...
    parser.add_argument(
<<<<<<< HEAD
        "--display-data", action="store_true", help="Show action data in Rerun."
    )
=======
        "--debug-transition",
        action="store_true",
        help="Print the final EnvTransition and the action sent to motors.",
    )
    parser.add_argument(
        "--torque-disable",
        action="store_true",
        help="Disable motor torque after connecting and skip sending actions to the robot.",
    )
...
<<<<<<< HEAD
    return RobotProcessorPipeline[tuple[RobotAction, RobotObservation], RobotAction](
        steps=[
            ...
            EEBoundsAndSafety(
                end_effector_bounds={
                    "min": list(workspace_min),
                    "max": list(workspace_max),
                },
...
<<<<<<< HEAD
    max_relative_target = (
        args.max_relative_target if args.max_relative_target > 0 else None
    )
=======
...
=======
        if args.torque_disable:
            ...
            print("Motor torque disabled. Actions will be computed and logged, but not sent.")

        print("Starting SO-101 gamepad teleoperation. Press Ctrl-C to stop.")
        old_logged_env_transition = None
>>>>>>> 9e95a9325284e42689995885da3b36011644005e
        while True:
...
=======
            ...
            try:
                ...
                if args.debug_transition:
                    for final_transition in gamepad_to_joint_processor.step_through((gamepad_action, robot_obs)):
                        pass
                    if final_transition is None:
                        raise ValueError("Processor pipeline did not produce a transition.")
                    joint_action = transition_to_robot_action(final_transition)
                else:
                    joint_action = gamepad_to_joint_processor((gamepad_action, robot_obs))
            except ValueError as exc:
                logging.warning("Skipping unsafe gamepad command: %s", exc)
            ...
            if joint_action is not None:
                if args.torque_disable:
                    sent_action = joint_action
                else:
                    sent_action = robot.send_action(joint_action)
                if args.debug_transition and final_transition != old_logged_env_transition:
                    if final_transition is not None:
                        print("\nFinal EnvTransition after InverseKinematicsEEToJoints:")
                        print(pformat(final_transition, sort_dicts=False), flush=True)
                        old_logged_env_transition = final_transition
                    # print(
                    #     "\nComputed action with torque disabled:"
                    #     if args.torque_disable
                    #     else "\nAction sent to motors:"
                    # )
                    # print(pformat(sent_action, sort_dicts=False), flush=True)
                    # old_logged_action = dict(sent_action)
>>>>>>> 9e95a9325284e42689995885da3b36011644005e
                if args.display_data:
